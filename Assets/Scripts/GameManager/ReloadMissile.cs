...
public class ReloadMissile : MonoBehaviour
{
    float fillAmount = 1;
    ...
    void Start()
    {
        ...
        count.ChangeCount(playerScript.missileCount);
    }

    void Update()
    {
        //Function for starting Reload GFX;
        //MissileFlag is false when Missile is Fired.
        if (!actionManager.missileFlag && !reloadBool)
        {
            count.ChangeCount(playerAction.currentMissileCount);
            reloadBool = true;
            fillAmount = 0;
            InvokeRepeating ("Reload", 0, 0.05f);
        }
        
        if (playerAction.currentMissileCount == 0)
        {
    ...
    void Reload()
    {
        ...
        {
            reloadBool = false;
            actionManager.missileFlag = true;
            CancelInvoke("Reload");
        }
    }
}
